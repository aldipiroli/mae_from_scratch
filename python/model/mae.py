...
class MAE(nn.Module):
    def __init__(
        ...
        self.mask_tokens = nn.Parameter(torch.zeros(1, 1, self.embed_size), requires_grad=True)
    ...
    def forward(self, x):
        ...
        mask_tokens = self.mask_tokens.expand(b, self.num_patches, -1)
        x_encoder_w_masked_tokens = torch.cat([x_encoder, mask_tokens], 1)

        pred_token_mask = torch.ones((b, self.num_patches, self.embed_size), device=x.device)
